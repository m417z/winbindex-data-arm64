...
def main():
    ...
    while True:
        ...
        # Stop once we got less than 25 non-update files from VirusTotal.
        # Otherwise, continue as long as there are new updates.
        match = re.match(r'Updated info of (\d+) files from VirusTotal$', pr_title)
        if match and int(match.group(1)) < 25 and not is_handling_update_in_info_progress_virustotal():
            print('Done')
            return
